...
router.post("/register", async (req, res) => {
  const { email, first_name, last_name, password, brtel , dob } = req.body;
  const hashedPassword = await bcrypt.hash(password, 10);
  
  db.query("INSERT INTO users (email, first_name, last_name, password, brtel ,dob, role) VALUES (?, ?, ?, ?, ?, ?, ?)", 
    [email, first_name, last_name, hashedPassword, brtel, dob, "user"], (err) => {
